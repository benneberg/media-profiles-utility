...
import { FileVideo, Trash2, Check, ExternalLink, Plus, Square, CheckSquare } from "lucide-react";
...
export default function AssetLibrary() {
  ...
  const handleSelectAsset = async (asset: any) => {
    if (isBatchMode) {
      toggleSelectAsset(asset.assetId);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3">
        ...
        {/* Batch Mode Toggle */}
        <div className="flex items-center justify-between p-2 bg-black/5 border border-black/10">
          <span className="text-[10px] font-black text-black uppercase tracking-widest">
            {isBatchMode ? `Batch Processing (${selectedAssetIds.length} Selected)` : "Single File Mode"}
          </span>
          <button
            onClick={() => setBatchMode(!isBatchMode)}
            className={cn(
              "px-3 py-1 text-[10px] font-black uppercase tracking-wider border border-black transition-all shadow-brutal-sm",
              isBatchMode 
                ? "bg-black text-white hover:bg-white hover:text-black" 
                : "bg-white text-black hover:bg-black hover:text-white"
            )}
          >
            {isBatchMode ? "Exit Batch" : "Enable Batch"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {assets.map((asset, index) => {
          const isSelectedInBatch = selectedAssetIds.includes(asset.assetId);
          const isSelectedInSingle = currentAsset?.assetId === asset.assetId;
          const isActive = isBatchMode ? isSelectedInBatch : isSelectedInSingle;

          return (
            <div
              key={asset.assetId || asset.filename || index}
              className={cn(
            ...
            >
              ...
              {!isBatchMode && isSelectedInSingle && (
